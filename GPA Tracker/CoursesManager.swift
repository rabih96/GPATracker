...
func getHighestGPAGrade() -> Float {
    var gradesArray = [Float]()
    let gradesLettersArray = ["A", "B", "C", "D", "A+", "B+", "C+", "D+", "A-", "B-", "C-", "D-", "F"]

    for gradeLetter in gradesLettersArray {
        gradesArray.append(gradeToValue(gradeLetter))
    }

    return gradesArray.max()!
}

func getLowestGPAGrade() -> Float {
    var gradesArray = [Float]()
    let gradesLettersArray = ["A", "B", "C", "D", "A+", "B+", "C+", "D+", "A-", "B-", "C-", "D-", "F"]
    ...
    return gradesArray.min()!
}

func calculateRequiredGPA() -> Float {
    let result = (userDefaults.float(forKey: "Desired Cumulative GPA") * userDefaults.float(forKey: "Major Total Credits") - Float(getCumulativeGPA())! * Float(getTotalCreditsFinished())) / (userDefaults.float(forKey: "Major Total Credits") - Float(getTotalCreditsFinished()))
    if result.isNaN {
        return 0
    }else{
        return result
